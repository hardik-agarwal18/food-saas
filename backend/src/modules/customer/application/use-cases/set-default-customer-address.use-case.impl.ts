...
export interface ISetDefaultCustomerAddressUseCase {
  execute(params: { addressId: string; userId: string }): Promise<void>;
}

@injectable()
export class SetDefaultCustomerAddressUseCaseImpl implements ISetDefaultCustomerAddressUseCase {
  constructor(
  ...
  async execute(params: { addressId: string; userId: string }): Promise<void> {
    ...
    if (address.getCustomerId() !== customer.getId()) {
      throw new AuthorizationError('Unauthorized to modify this address');
    }

    await this.addressRepo.resetDefaultAddress(customer.getId());
