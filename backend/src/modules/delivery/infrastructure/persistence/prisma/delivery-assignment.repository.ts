...
@injectable()
export class DeliveryAssignmentRepositoryImpl
  extends BaseRepository
  implements IDeliveryAssignmentRepository
{
  ...
  async findById(id: string): Promise<DeliveryAssignment | null> {
    const record = await this.execute(() =>
      this.prisma.deliveryAssignment.findUnique({ where: { id } }),
    );
  ...
  async findByOrderId(orderId: string): Promise<DeliveryAssignment[]> {
    const records = await this.execute(() =>
      this.prisma.deliveryAssignment.findMany({ where: { orderId } }),
    );
