...
export function MenuItemModal({ item, restaurantId, isOpen, onClose }: MenuItemModalProps) {
  ...
  const [selectedModifiers, setSelectedModifiers] = useState<Omit<OrderItemModifier, 'id' | 'orderItemId'>[]>([]);
  ...
  const handleAddModifier = (modItem: ModifierItemData) => {
    ...
    const exists = selectedModifiers.find(m => m.menuModifierItemId === modItem.id);
    if (exists) {
      setSelectedModifiers(selectedModifiers.filter(m => m.menuModifierItemId !== modItem.id));
    } else {
      setSelectedModifiers([...selectedModifiers, {
        menuModifierItemId: modItem.id,
        name: modItem.name,
        price: modItem.price,
      }]);
    }
  ...
  const handleAddToCart = () => {
    try {
      addItem({
        menuItem: item,
        quantity,
        modifiers: selectedModifiers as any, // Cast since cart modifiers lack ids until ordered
        specialInstructions
      }, restaurantId);
      onClose();
                ...
                <div className="space-y-2">
                  {group.items?.map((modItem) => (
                    <div key={modItem.id} className="flex items-center space-x-2">
                      <Checkbox 
                        id={modItem.id} 
                        checked={!!selectedModifiers.find(m => m.menuModifierItemId === modItem.id)}
